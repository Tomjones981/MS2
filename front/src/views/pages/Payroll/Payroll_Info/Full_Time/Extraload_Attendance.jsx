...
const Extraload_Attendance = () => {
    const { facultyId } = useParams();
    const location = useLocation();
    const { selectedDates, rateValue, totalHours, amountGross, adjustmentAmount, totalLate, withholdingTax, netPay} = location.state || { selectedDates: [] };
    ...
    const printTable = () => {
      ...
      printWindow.document.write('<thead><tr><th>RATE</th><th>TOTAL HOURS</th><th>GROSS AMOUNT</th><th>ADJUSTMENT</th><th>TOTAL LATE</th><th>TAX</th><th>NETPAY</th></tr></thead><tbody>');
      
      printWindow.document.write('<tr>');
      printWindow.document.write(`<td>₱${rateValue ? parseFloat(rateValue).toFixed(2) : "N/A"}</td>`);
      printWindow.document.write(`<td>${totalHours ? parseFloat(totalHours).toFixed(2) : "N/A"}</td>`);
      printWindow.document.write(`<td>₱${amountGross ? parseFloat(amountGross).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : "N/A"}</td>`); 
      printWindow.document.write(`<td>₱${adjustmentAmount ? adjustmentAmount : "0"}</td>`);
      printWindow.document.write(`<td>${totalLate ? totalLate : "0"}</td>`);
    ...
    <>
          ...
          <table className='table-auto w-full text-sm text-left text-gray-500 dark:text-gray-400 border-collapse border border-slate-200'>
            <thead className='sticky -top-1 text-xs text-gray-100 bg-gray-700 dark:bg-gray-700 dark:text-gray-200 border-collapse border border-slate-200 dark:border-slate-700'>
              <tr>
                ...
                <th scope="col" className="px-6 py-3 border border-slate-300 dark:border-slate-600 uppercase">Rate</th>
                <th scope="col" className="px-6 py-3 border border-slate-300 dark:border-slate-600 uppercase">Total Hours</th>
                <th scope="col" className="px-6 py-3 border border-slate-300 dark:border-slate-600 uppercase">Gross</th>
                <th scope="col" className="px-6 py-3 border border-slate-300 dark:border-slate-600 uppercase">Adjustment</th>
            ...
            <tbody className=' '>
              <tr>
                ...
                <td className='border border-slate-300 px-6 py-4 dark:border-slate-600 dark:text-gray-400 overflow-hidden text-ellipsis whitespace-nowrap font-bold text-center'>₱{rateValue ? parseFloat(rateValue).toFixed(2) : "N/A"}</td>
                <td className='border border-slate-300 px-6 py-4 dark:border-slate-600 dark:text-gray-400 overflow-hidden text-ellipsis whitespace-nowrap font-bold text-center'>{totalHours ? parseFloat(totalHours).toFixed(2) : "N/A"}</td>
                <td className='border border-slate-300 px-6 py-4 dark:border-slate-600 dark:text-gray-400 overflow-hidden text-ellipsis whitespace-nowrap font-bold text-center'>₱{amountGross ? parseFloat(amountGross).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : "N/A"}</td>
                ...
                <td className='border border-slate-300 px-6 py-4 dark:border-slate-600 dark:text-gray-400 overflow-hidden text-ellipsis whitespace-nowrap font-bold text-center'>{totalLate ? totalLate : "0"}</td>
                <td className='border border-slate-300 px-6 py-4 dark:border-slate-600 dark:text-gray-400 overflow-hidden text-ellipsis whitespace-nowrap font-bold text-center'>₱{withholdingTax ? parseFloat(withholdingTax).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : "N/A"}</td>
      ...
      <div>
        <div className="col-span-5 border border-gray-200 bg-white p-6 rounded-md shadow-md dark:border-slate-700 dark:bg-gray-800 dark:text-gray-200">
          <div className="flex flex-col items-start mb-5">
            <div className="flex w-full justify-center items-center">
              <h1 className="font-semibold text-xl dark:text-white uppercase"> EXTRA-LOAD Attendance Details</h1>
              {/* <div className="flex space-x-2">  
                <button type="button" className="flex items-center justify-between py-2 px-4 text-white bg-gray-500 rounded-md hover:bg-gray-600 focus:outline-none focus:ring-4 focus:ring-gray-300 dark:bg-gray-600 dark:hover:bg-gray-700 dark:focus:ring-gray-800" onClick={printTable} >Print <img src={printIcon} alt="Print" className="  w-5 h-5" /> </button>
              </div> */}
            </div>
...
export default Extraload_Attendance