...
const Part_Time_Payroll_Attendance = () => {
    const { facultyId } = useParams();
    const location = useLocation();
    const { selectedDates, totalLate, rateValue, HoursWork, amountGross, adjustmentAmount, withholdingTax, netPay } = location.state || { selectedDates: [] };
    ...
    const printTable = () => {
      ...
      printWindow.document.write(`<td>${HoursWork ? parseFloat(HoursWork).toFixed(2) : "N/A"}</td>`);
          ...
          <table className='table-auto w-full text-sm text-left text-gray-500 dark:text-gray-400 border-collapse border border-slate-200'>
            ...
            <tbody className=' '>
              <tr>
                ...
                <td className='border border-slate-300 px-6 py-4 dark:border-slate-600 dark:text-gray-400 overflow-hidden text-ellipsis whitespace-nowrap font-bold text-center'>{HoursWork ? parseFloat(HoursWork).toFixed(2) : "N/A"}</td>
...
export default Part_Time_Payroll_Attendance