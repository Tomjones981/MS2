...
const Try = () => {
    ...
    const handleMonthChange = async (date, dateString) => {
        ...
        try {
            ...
            if (dateRanges.length > 0) {
                // Automatically fetch data for the first date range
    ...
    const fetchData = async (dateFrom, dateTo) => {
        try {
            const response = await axiosClient.post('/getPayrollPartTimeHistory', {
                date_from: dateFrom,
                date_to: dateTo,
            });
    ...
    const buttons = [
        ...
        { label: 'Part Time', route: '/admin/adjustment/part/time', icon: <FaUserClock />, color: 'bg-gray-200 hover:bg-gray-300 text-gray-800' },
            ...
            <div className="bg-gray-800 p-6 m-1 rounded-lg shadow-md">
                <div className="flex flex-col items-start mb-5">
                    <div className="flex w-full justify-between items-center">
                        <h1 className="font-bold text-2xl text-white uppercase tracking-wide">HISTORY</h1>
                    </div>
                        ...
                        <table className="table-auto w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 border-collapse border border-slate-200">
                            ...
                            <tbody> 
                                ...
                                <tr key={`${payroll.faculty_id}-${index}`}  className="bg-white dark:bg-gray-800 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 border-b border-slate-200">
                                    ...
                                    <td className="px-6 py-3 border border-slate-300 dark:border-slate-600 text-center">₱{formatNumber(payroll.rate_value)}</td> 
                                    <td className="px-6 py-3 border border-slate-300 dark:border-slate-600 text-center">{payroll.hours_or_days}</td>
                                    <td className="px-6 py-3 border border-slate-300 dark:border-slate-600 text-center">₱{formatNumber(payroll.gross_amount)}</td>
                                    
                                    <td className="px-6 py-3 border border-slate-300 dark:border-slate-600 text-center">{payroll.late}</td>
                                    <td className="px-6 py-3 border border-slate-300 dark:border-slate-600 text-center">₱{payroll.tax}</td>
                                    <td className="px-6 py-3 border border-slate-300 dark:border-slate-600 text-center">₱{formatNumber(payroll.netpay)}</td>
