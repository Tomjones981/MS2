...
const PT_Regular_History = () => {
    ...
    const handleMonthChange = async (date, dateString) => {
        ...
        try {
            const response = await axiosClient.post('/getDateRangesForMonth', { month: dateString });
    ...
    const fetchData = async (dateFrom, dateTo) => {
        setLoading(true); 
        try {
            const response = await axiosClient.post('/get-payroll-history/part/time/regular', {
                date_from: dateFrom,
                date_to: dateTo,
            });
    ...
    const printTable = () => {
        ...
        const dateFrom = selectedMonth ? selectedMonth.startOf('month').format('YYYY-MM-DD') : '';
        const dateTo = selectedMonth ? selectedMonth.endOf('month').format('YYYY-MM-DD') : '';
      
        const formattedDateRange = dateFrom && dateTo ? formatDateRange(dateFrom, dateTo) : 'Date range not selected';
        ...
        printWindow.document.write('<h2>PT-Regular Payroll  </h2>'); 
        printWindow.document.write(`
            <div class="flex items-center justify-center border bg-white p-2 mb-10 rounded-md shadow-md -m-5">
                ...
                <div class="text-center">
                    ...
                    <p style="font-size: 16px; font-weight: 600;  uppercase;">${formattedDateRange}</p>
                </div>
            </div>
        `);
        printWindow.document.write('<table>');
        printWindow.document.write('<thead><tr><th class="text-center">Faculty Name</th><th class="text-center">Rate</th><th class="text-center">Worked Hours</th><th class="text-center">Gross</th><th class="text-center">Late</th><th class="text-center">W/holding Tax</th><th class="text-center">Adjustment</th><th class="text-center">Net Pay</th><th class="text-center">Signature</th></tr></thead>');
        ...
        (data || []).forEach(payroll => {
            const adjustedNetPay = (payroll.adjusted_netpay === 0 || payroll.adjusted_netpay == null) ? payroll.netpay : payroll.adjusted_netpay;
          
            printWindow.document.write('<tr>');
            printWindow.document.write(`<td class="text-left">${payroll.full_name}</td>`);
            printWindow.document.write(`<td class="text-right">₱${formatNumber(payroll.rate_value)}</td>`);
            totalRate += parseFloat(payroll.rate_value || 0);
            printWindow.document.write(`<td class="text-center">${formatNumber((parseFloat(payroll.hours_or_days) || 0).toFixed(2))}</td>`);
        ...
        printWindow.document.write(`
            <tfoot>
                <tr>
                    ...
                    <td class="text-right font-bold">₱${formatNumber(totalRate)}</td>
    ...
    function formatDateRange(dateFrom, dateTo) {
      ...
      const filteredPayrollData = data ? data.filter(payroll => 
        payroll.full_name.toLowerCase().includes(searchTerm.toLowerCase())
    ) : [];

    const buttons = [
        { label: 'Full Time', route: '/admin/history/full/time', icon: <FaUserTie />, color: 'bg-gray-600 hover:bg-gray-500 text-gray-200' },
        ...
        { label: 'PT-Regular', route: '/admin/history/part/time/regular', icon: <FaUserPlus />, color: 'bg-gray-200 hover:bg-gray-300 text-gray-800' },
    ...
    return (
        <div className="mt-2 col-span-5 p-6 rounded-lg shadow-lg bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 transition-all duration-300">
            ...
            <div className='grid grid-cols-10 mt-10 mb-2 gap-4'>
                ...
                <div className="max-h-[22rem] overflow-y-auto mb-4">
                    ...
                    <table className="table-auto w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 border-collapse border border-slate-200">
                        <thead className="sticky -top-1 text-xs text-gray-100 bg-gray-600 dark:bg-gray-700 dark:text-gray-200 border-collapse border border-slate-200">
                            ...
                            <th scope="col" className="px-6 py-3 border border-slate-300 dark:border-slate-600 text-center">Worked Hours</th>
                        ...
                        <tbody>
                            ...
                            filteredPayrollData?.map((payroll, index) => {
                                ...
                                <tr key={`${payroll.faculty_id}-${index}`} className="bg-white dark:bg-gray-800 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 border-b border-slate-200">
                                    ...
                                    <td className="px-6 py-3 border border-slate-300 dark:border-slate-600 font-medium text-gray-500 whitespace-nowrap dark:text-gray-400 text-center">₱{formatNumber(payroll.rate_value)}</td>
                                    <td className="px-6 py-3 border border-slate-300 dark:border-slate-600 font-medium text-gray-500 whitespace-nowrap dark:text-gray-400 text-center"> {formatNumber((parseFloat(payroll.hours_or_days) || 0).toFixed(2))}</td>
...
export default PT_Regular_History;
