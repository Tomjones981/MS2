...
const Program_Heads_Adjustments = () => {
    ...
    const handleDateChange = (dates) => {
      ...
      const fetchPayrollHistory = async (e) => {
        ...
        try {
          setLoadingGen(true);
          const response = await axiosClient.get('/get-payroll-adjustment/program/heads', {
            params: {
              date_from: dateFrom,
              date_to: dateTo,
            },
    ...
    const buttons = [
        ...
        { label: 'Program Heads', route: '/admin/adjustment/program/heads', icon: <FaUserFriends />, color: 'bg-gray-200 hover:bg-gray-300 text-gray-800' },
    ];
              ...
              <table className="table-auto w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 border-collapse border border-slate-200">
                <thead className="sticky -top-1 text-xs text-gray-100 bg-gray-600 dark:bg-gray-700 dark:text-gray-200 border-collapse border border-slate-200">
                  <tr>
                    ...
                    <th scope="col" className="px-6 py-3 border border-slate-300 dark:border-slate-600">Worked Days</th>
                ...
                <tbody> 
                    {filteredPayrollData.map((payroll, index) => (
                      <tr key={`${payroll.faculty_id}-${index}`}  className="bg-white dark:bg-gray-800 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 border-b border-slate-200">
                        ...
                        <td className="px-6 py-3 border border-slate-300 dark:border-slate-600 text-center">₱{formatNumber(payroll.monthly_rate)}</td> 
...
export default Program_Heads_Adjustments;
