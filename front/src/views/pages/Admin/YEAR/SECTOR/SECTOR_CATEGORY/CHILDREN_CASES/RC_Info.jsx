...
const RC_Info = () => {
    ...
    const fetchRcInfo = async (SubCatId) => {
        setLoading(true);
        try {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            fetchRcInfo(SubCatId);
        } catch (error) {
            if (error.response?.status === 422) {
                setErrors(error.response.data.errors);
            } else {
                console.error("Error Creating RC info", error);
    ...
    const fetchRcLocations = async () => {
        try {
            ...
            console.error("Error fetching RC Sex:", error);
        }
    };

    const [sex, setSex] = useState([]);
    const [selectedSex, setSelectedSex] = useState("");
    const fetchRcSex = async () => {
        try {
    ...
    const filteredRcInfo = rcInfo.filter(rc => {
        const query = searchQuery.toLowerCase();
        const matchesSearch = rc.code_name.toLowerCase().includes(query);
        
        const matchesLocation = selectedLocation ? rc.locations === selectedLocation : true;
        const matchesSex = selectedSex ? rc.sex === selectedSex : true;
        const matchesAge = selectedAge ? rc.age === selectedAge : true;
    
        return matchesSearch && matchesLocation && matchesSex && matchesAge;
    });
    const [currentPage, setCurrentPage] = useState(1);
    const [pageSize, setPageSize] = useState(10);
    const paginatedData = filteredRcInfo.slice(
    ...
    const buttons = [
           ...
           { label: 'RC', route: '/brgy-sectors/sub-category/rc/4', icon: <ShieldAlert  />, color: 'bg-gray-200 hover:bg-gray-300 text-gray-800' },
                ...
                <div className="bg-gray-800 p-6 m-1 rounded-lg shadow-md">  
                        <div className="flex flex-col items-start mb-5">
                          <div className="flex w-full justify-between items-center">
                            <h1 className="font-bold text-2xl text-white uppercase tracking-wide">Children Cases - RC</h1>  
                            ...
                            <table className="w-full">
                                ...
                                <tbody className='divide-y divide-gray-100 dark:divide-gray-700 dark:bg-gray-800'>
                                    {loading ? (
                                        ...
                                        ) : paginatedData.filter(rc => rc.children_case_type === "RC").length === 0 ? (
                                            <tr>
                                        ...
                                        paginatedData
                                        .filter(rc => rc.children_case_type === "CAR")
                                        .map((rc) => (
                                        <tr key={rc.id} className="bg-white dark:bg-gray-800"> 
                                            <td className=" p-3 text-sm text-gray-700   dark:text-gray-200">{rc.code_name}</td> 
                                            <td className=" p-3 text-sm text-gray-700   dark:text-gray-200">{rc.locations}</td> 
                                            <td className=" p-3 text-sm text-gray-700   dark:text-gray-200">{rc.age}</td> 
                                            <td className=" p-3 text-sm text-gray-700   dark:text-gray-200">{rc.sex}</td>  
                                            {/* <td className=" p-3 text-sm text-gray-700   dark:text-gray-200">{rc.religion}</td>   */}
                                            {/* <td className=" p-3 text-sm text-gray-700   dark:text-gray-200">{rc.educational_attainment}</td>   */}
                                            {/* <td className=" p-3 text-sm text-gray-700   dark:text-gray-200">{rc.educational_status}</td>   */}
                                            {/* <td className=" p-3 text-sm text-gray-700   dark:text-gray-200">{rc.ethnic_affiliation}</td>   */}
                                            {/* <td className=" p-3 text-sm text-gray-700   dark:text-gray-200">{rc.four_ps_beneficiary}</td>   */}
                                            <td className=" p-3 text-sm text-gray-700   dark:text-gray-200">{rc.case}</td>  
                                            {/* <td className=" p-3 text-sm text-gray-700   dark:text-gray-200">{rc.case_status}</td>   */}
                                            {/* <td className=" p-3 text-sm text-gray-700   dark:text-gray-200">{rc.perpetrator}</td>   */}
                                            <td className=" p-3 text-sm text-gray-700   dark:text-gray-200">{rc.interventions}</td>   
                                            <td className="p-3 text-sm text-gray-700 whitespace-nowrap flex space-x-2">
                                                <button  onClick={() => handleViewClick(rc)}   className="bg-white px-3 py-1 border rounded-md text-blue-500 hover:text-blue-700 dark:bg-gray-800 transform scale-100 hover:scale-110 transition-all duration-300"><FaEye /></button>
                                                <button  onClick={() => handleEditClick(rc)}  className="bg-white px-3 py-1 border rounded-md text-green-500 hover:text-green-700 dark:bg-gray-800 transform scale-100 hover:scale-110 transition-all duration-300"><FaRegEdit /></button>
                                            </td>  
                                        </tr>
                                    ))
                                )}
