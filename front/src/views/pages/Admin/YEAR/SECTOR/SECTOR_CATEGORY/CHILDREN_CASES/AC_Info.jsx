...
const AC_Info = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            fetchAcInfo(SubCatId);
        } catch (error) {
            if (error.response?.status === 422) {
                setErrors(error.response.data.errors);
            } else {
                console.error("Error Creating AC info", error);
    ...
    const fetchAcLocations = async () => {
        try {
            ...
            console.error("Error fetching AC Sex:", error);
        }
    };

    const [sex, setSex] = useState([]);
    const [selectedSex, setSelectedSex] = useState("");
    const fetchAcSex = async () => {
        try {
    ...
    const filteredAcInfo = acInfo.filter(ac => {
        const query = searchQuery.toLowerCase();
        const matchesSearch = ac.code_name.toLowerCase().includes(query);
        
        const matchesLocation = selectedLocation ? ac.locations === selectedLocation : true;
        const matchesSex = selectedSex ? ac.sex === selectedSex : true;
        const matchesAge = selectedAge ? ac.age === selectedAge : true;
    
        return matchesSearch && matchesLocation && matchesSex && matchesAge;
    });
    const [currentPage, setCurrentPage] = useState(1);
    const [pageSize, setPageSize] = useState(10);
    const paginatedData = filteredAcInfo.slice(
    ...
    const buttons = [
           ...
           { label: 'CICL', route: '/brgy-sectors/sub-category/cicl/4', icon: < Users />, color: 'bg-gray-600 hover:bg-gray-500 text-gray-200' },
           ...
           { label: 'AC', route: '/brgy-sectors/sub-category/ac/4', icon: <Hand  />, color: 'bg-gray-200 hover:bg-gray-300 text-gray-800' }, 
       ];
                ...
                <div className="bg-gray-800 p-6 m-1 rounded-lg shadow-md">  
                        <div className="flex flex-col items-start mb-5">
                          <div className="flex w-full justify-between items-center">
                            <h1 className="font-bold text-2xl text-white uppercase tracking-wide">Children Cases - AC</h1>  
                            ...
                            <table className="w-full">
                                ...
                                <tbody className='divide-y divide-gray-100 dark:divide-gray-700 dark:bg-gray-800'>
                                    {loading ? (
                                        ...
                                        ) : paginatedData.filter(ac => ac.children_case_type === "AC").length === 0 ? (
                                            <tr>
                                        ...
                                        paginatedData
                                        .filter(ac => ac.children_case_type === "AC")
                                        .map((ac) => (
                                        <tr key={ac.id} className="bg-white dark:bg-gray-800"> 
                                            <td className=" p-3 text-sm text-gray-700   dark:text-gray-200">{ac.code_name}</td> 
                                            <td className=" p-3 text-sm text-gray-700   dark:text-gray-200">{ac.locations}</td> 
                                            <td className=" p-3 text-sm text-gray-700   dark:text-gray-200">{ac.age}</td> 
                                            <td className=" p-3 text-sm text-gray-700   dark:text-gray-200">{ac.sex}</td>  
                                            {/* <td className=" p-3 text-sm text-gray-700   dark:text-gray-200">{ac.religion}</td>   */}
                                            {/* <td className=" p-3 text-sm text-gray-700   dark:text-gray-200">{ac.educational_attainment}</td>   */}
                                            {/* <td className=" p-3 text-sm text-gray-700   dark:text-gray-200">{ac.educational_status}</td>   */}
                                            {/* <td className=" p-3 text-sm text-gray-700   dark:text-gray-200">{ac.ethnic_affiliation}</td>   */}
                                            {/* <td className=" p-3 text-sm text-gray-700   dark:text-gray-200">{ac.four_ps_beneficiary}</td>   */}
                                            <td className=" p-3 text-sm text-gray-700   dark:text-gray-200">{ac.case}</td>  
                                            {/* <td className=" p-3 text-sm text-gray-700   dark:text-gray-200">{ac.case_status}</td>   */}
                                            {/* <td className=" p-3 text-sm text-gray-700   dark:text-gray-200">{ac.perpetrator}</td>   */}
                                            <td className=" p-3 text-sm text-gray-700   dark:text-gray-200">{ac.interventions}</td>   
                                            <td className="p-3 text-sm text-gray-700 whitespace-nowrap flex space-x-2">
                                                <button  onClick={() => handleViewClick(ac)}   className="bg-white px-3 py-1 border rounded-md text-blue-500 hover:text-blue-700 dark:bg-gray-800 transform scale-100 hover:scale-110 transition-all duration-300"><FaEye /></button>
                                                <button  onClick={() => handleEditClick(ac)}  className="bg-white px-3 py-1 border rounded-md text-green-500 hover:text-green-700 dark:bg-gray-800 transform scale-100 hover:scale-110 transition-all duration-300"><FaRegEdit /></button>
                                            </td>  
                                        </tr>
                                    ))
                                )}
