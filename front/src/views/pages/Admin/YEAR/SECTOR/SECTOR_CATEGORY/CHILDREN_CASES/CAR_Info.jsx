...
const CAR_Info = () => {
    ...
    const fetchCarInfo = async (SubCatId) => {
        setLoading(true);
        try {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            fetchCarInfo(SubCatId);
        } catch (error) {
            if (error.response?.status === 422) {
                setErrors(error.response.data.errors);
            } else {
                console.error("Error Creating CAR info", error);
    ...
    const fetchCarLocations = async () => {
        try {
            ...
            console.error("Error fetching CAR Sex:", error);
        }
    };

    const [sex, setSex] = useState([]);
    const [selectedSex, setSelectedSex] = useState("");
    const fetchCarSex = async () => {
        try {
    ...
    const filteredCarInfo = carInfo.filter(car => {
        const query = searchQuery.toLowerCase();
        const matchesSearch = car.code_name.toLowerCase().includes(query);
        
        const matchesLocation = selectedLocation ? car.locations === selectedLocation : true;
        const matchesSex = selectedSex ? car.sex === selectedSex : true;
        const matchesAge = selectedAge ? car.age === selectedAge : true;
    
        return matchesSearch && matchesLocation && matchesSex && matchesAge;
    });
    const [currentPage, setCurrentPage] = useState(1);
    const [pageSize, setPageSize] = useState(10);
    const paginatedData = filteredCarInfo.slice(
    ...
    const buttons = [
           ...
           { label: 'CAR', route: '/brgy-sectors/sub-category/car/4', icon: <AlertTriangle  />, color: 'bg-gray-200 hover:bg-gray-300 text-gray-800' },
                ...
                <div className="bg-gray-800 p-6 m-1 rounded-lg shadow-md">  
                        <div className="flex flex-col items-start mb-5">
                          <div className="flex w-full justify-between items-center">
                            <h1 className="font-bold text-2xl text-white uppercase tracking-wide">Children Cases - CAR</h1>  
                            ...
                            <table className="w-full">
                                ...
                                <tbody className='divide-y divide-gray-100 dark:divide-gray-700 dark:bg-gray-800'>
                                    {loading ? (
                                        ...
                                        ) : paginatedData.filter(car => car.children_case_type === "CAR").length === 0 ? (
                                            <tr>
                                        ...
                                        paginatedData
                                        .filter(car => car.children_case_type === "CAR")
                                        .map((car) => (
                                        <tr key={car.id} className="bg-white dark:bg-gray-800"> 
                                            <td className=" p-3 text-sm text-gray-700   dark:text-gray-200">{car.code_name}</td> 
                                            <td className=" p-3 text-sm text-gray-700   dark:text-gray-200">{car.locations}</td> 
                                            <td className=" p-3 text-sm text-gray-700   dark:text-gray-200">{car.age}</td> 
                                            <td className=" p-3 text-sm text-gray-700   dark:text-gray-200">{car.sex}</td>  
                                            {/* <td className=" p-3 text-sm text-gray-700   dark:text-gray-200">{car.religion}</td>   */}
                                            {/* <td className=" p-3 text-sm text-gray-700   dark:text-gray-200">{car.educational_attainment}</td>   */}
                                            {/* <td className=" p-3 text-sm text-gray-700   dark:text-gray-200">{car.educational_status}</td>   */}
                                            {/* <td className=" p-3 text-sm text-gray-700   dark:text-gray-200">{car.ethnic_affiliation}</td>   */}
                                            {/* <td className=" p-3 text-sm text-gray-700   dark:text-gray-200">{car.four_ps_beneficiary}</td>   */}
                                            <td className=" p-3 text-sm text-gray-700   dark:text-gray-200">{car.case}</td>  
                                            {/* <td className=" p-3 text-sm text-gray-700   dark:text-gray-200">{car.case_status}</td>   */}
                                            {/* <td className=" p-3 text-sm text-gray-700   dark:text-gray-200">{car.perpetrator}</td>   */}
                                            <td className=" p-3 text-sm text-gray-700   dark:text-gray-200">{car.interventions}</td>   
                                            <td className="p-3 text-sm text-gray-700 whitespace-nowrap flex space-x-2">
                                                <button  onClick={() => handleViewClick(car)}   className="bg-white px-3 py-1 border rounded-md text-blue-500 hover:text-blue-700 dark:bg-gray-800 transform scale-100 hover:scale-110 transition-all duration-300"><FaEye /></button>
                                                <button  onClick={() => handleEditClick(car)}  className="bg-white px-3 py-1 border rounded-md text-green-500 hover:text-green-700 dark:bg-gray-800 transform scale-100 hover:scale-110 transition-all duration-300"><FaRegEdit /></button>
                                            </td>  
                                        </tr>
                                    ))
                                )}
