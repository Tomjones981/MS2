...
class FacultyController extends Controller
{
    ...
    public function showFacByID($id)
    {
        ...
        return response()->json([
            ...
            'subjects' => $faculty->unit->pluck('subjects'),
            'teaching_units' => $faculty->unit->pluck('teaching_units'),
            'monday' => $faculty->unit->pluck('monday'),
            'tuesday' => $faculty->unit->pluck('tuesday'),
            'wednesday' => $faculty->unit->pluck('wednesday'),
            'thursday' => $faculty->unit->pluck('thursday'),
            'friday' => $faculty->unit->pluck('friday'),
            'saturday' => $faculty->unit->pluck('saturday'),
        ]);
    }
    ...
    public function updateFaculty(Request $request, $id)
    {
        ...
        $request->validate([
            ...
            'teaching_units' => 'nullable|string',
        ...
        $teaching_units = explode(',', $request->input('teaching_units'));
        $monday = explode(',', $request->input('monday'));
        $tuesday = explode(',', $request->input('tuesday'));
        $wednesday = explode(',', $request->input('wednesday'));
        $thursday = explode(',', $request->input('thursday'));
        $friday = explode(',', $request->input('friday'));
        $saturday = explode(',', $request->input('saturday'));
        $faculty->unit()->delete(); 
        foreach ($subjects as $index => $subject) {
            Unit::create([
                'faculty_id' => $faculty->id,
                'subjects' => trim($subject),
                'teaching_units' => isset($teaching_units[$index]) ? trim($teaching_units[$index]) : null,
                'monday' => isset($monday[$index]) ? trim($monday[$index]) : null,
                'tuesday' => isset($tuesday[$index]) ? trim($tuesday[$index]) : null,
                'wednesday' => isset($wednesday[$index]) ? trim($wednesday[$index]) : null,
                'thursday' => isset($thursday[$index]) ? trim($thursday[$index]) : null,
                'friday' => isset($friday[$index]) ? trim($friday[$index]) : null,
                'saturday' => isset($saturday[$index]) ? trim($saturday[$index]) : null,
            ]);
        }
